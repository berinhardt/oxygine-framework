...
public:
   ...
   TouchEvent(eventType type, bool Bubbles = true, const Vector2& locPosition = Vector2(0, 0)) : Event(type, Bubbles), localPosition(
         locPosition), position(locPosition), mouseButton(MouseButton_Touch), pressure(1.0f), index(1), __clickDispatched(false), emitClick(
         true), wheelDirection(0, 0), __localScale(1.0f) {}
   ...
   static bool         isTouchEvent(int eventID) {
      return eventID > __FIRST && eventID < __LAST;
   }
