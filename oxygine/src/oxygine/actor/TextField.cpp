...
void TextField::setTrimLineHeight(bool trim) {
   _style.trimLineHeight = trim;
   needRebuild();
}
...
void TextField::setFont(const ResFont* font) {
   ...
   if (!_style.font) _style.font = _defaultFont;
   needRebuild();
}
...
void TextField::setStyle(const TextStyle& st) {
   ...
   if (st.hAlign == TextStyle::HALIGN_DEFAULT) _style.hAlign = halign;

   if (st.vAlign == TextStyle::VALIGN_DEFAULT) _style.vAlign = valign;

   if (st.fontSize == 0) _style.fontSize = size;
...
void TextField::matChanged() {
   if (_flags & flag_rebuild) return;

   if (!_root) return;

   logs::error("not optimal");
...
std::string dumpStyle(const TextStyle& s, bool onlydiff) {
   ...
   if (!onlydiff || (def.hAlign != s.hAlign)) stream << "hAlign=" << get_halign(s.hAlign);

   if (!onlydiff || (def.vAlign != s.vAlign)) stream << " vAlign=" << get_valign(s.vAlign);

   if (!onlydiff || (def.multiline != s.multiline)) stream << " " << (s.multiline ? "multiline" : "singleline");

   if (!onlydiff || (def.breakLongWords != s.breakLongWords)) stream << " " << (s.breakLongWords ? "breakLongWords=1" : "breakLongWords=0");

   if (!onlydiff || (def.kerning != s.kerning)) stream << " kerning=" << s.kerning;

   if (!onlydiff || (def.baselineScale != s.baselineScale)) stream << " baselineScale=" << s.baselineScale;

   if (!onlydiff || (def.linesOffset != s.linesOffset)) stream << " linesOffset=" << s.linesOffset;

   if (!onlydiff || (def.fontSize != s.fontSize)) stream << " fontSize=" << s.fontSize;

   if (!onlydiff || (def.options != s.options)) stream << " options=" << s.options;
...
std::string TextField::dump(const dumpOptions& options) const {
   ...
   if (st.size()) stream << " textStyle={" << st << "}";
