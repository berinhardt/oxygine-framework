...
class Actor : public EventDispatcher, public intrusive_list_item<spActor>, public Serializable {
   ...
   */
   ...
   void setAnchorAffectsOrigin(bool aff) {
   ...
   void setHasOwnBounds(bool enable = true) {
      ...
      if (enable) _flags |= flag_actorHasBounds;
   }
   void setBoundsNoChildren(bool enable = true) {
      _flags &= ~flag_boundsNoChildren;
      if (enable) _flags |= flag_boundsNoChildren;
   }
   /**by default actor with Alpha = 0 not clickable*/
   void setClickableWithZeroAlpha(bool enable) {
      _flags &= ~flag_clickableWithZeroAlpha;
      if (enable) _flags |= flag_clickableWithZeroAlpha;
   }

   /**Enables/Disables Touch events for Actor.*/
   void setTouchEnabled(bool enabled) {
      _flags &= ~flag_touchEnabled;
   ...
   void setTouchChildrenEnabled(bool enabled) {
      _flags &= ~flag_touchChildrenEnabled;
   ...
   template <class Prop>
   spTween addTween(const Prop& prop, timeMS duration, int loops = 1, bool twoSides = false, timeMS delay = 0, Tween::EASE ease = Tween::ease_linear) { return addTween(createTween(prop, duration, loops, twoSides, delay, ease)); }

   template <class Prop>
   spTween addTween(const Prop& prop, const TweenOptions& opt) { return addTween(createTween2(prop, opt)); }
