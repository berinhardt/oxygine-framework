...
void Actor::copyFrom(const Actor& src, cloneOptions opt) {
   ...
   if (src.__getName()) setName(src.getName());
}
...
std::string Actor::dump(const dumpOptions& opt) const {
   ...
   if (getUserData()) stream << " userData=" << (size_t)getUserData();

   if (!getVisible()) stream << " invisible";

   if (!getTouchEnabled()) stream << " touchEnabled=false";

   if (!getTouchChildrenEnabled()) stream << " touchChildrenEnabled=false";

   if (_flags & flag_actorHasBounds) stream << " flag_actorHasBounds";

   if (_flags & flag_clickableWithZeroAlpha) stream << " flag_clickableWithZeroAlpha";

   if (_flags & flag_cull) stream << " flag_cull";

   if (_flags & flag_anchorInPixels) stream << " flag_anchorInPixels";

   if (getAlpha() != 255) stream << " alpha=" << (int)getAlpha();

   if (getWidth() || getHeight()) stream << " size=(" << getWidth() << "," << getHeight() << ")";

   if (getPriority()) stream << " priority=" << getPriority();

   if (_extendedIsOn) stream << " extendedClickArea=" << (int)_extendedIsOn;

   if ((getX() != 0.0f) || (getY() != 0.0f)) stream << " pos=(" << getX() << "," << getY() << ")";

   if ((getScaleX() != 1.0f) || (getScaleY() != 1.0f)) stream << " scale=(" << getScaleX() << "," << getScaleY() << ")";

   if (getAnchor().x || getAnchor().y) stream << " anchor=(" << getAnchor().x << "," << getAnchor().y << ")";
   ...
   if (getClock()) stream << " " << getClock()->dump();
...
void Actor::_onGlobalTouchUpEvent(Event* ev) {
   ...
   if (te->index != _pressedButton[te->mouseButton]) return;
...
void Actor::_onGlobalTouchMoveEvent(Event* ev) {
   ...
   if (te->index != _overred) return;

   if (isDescendant(safeCast<Actor*>(ev->target.get()))) return;
...
void Actor::dispatchEvent(Event* event) {
   ...
   if (click.type) {
      ...
      if (me->emitClick) dispatchEvent(&click);
   }
}
...
Actor* Actor::getDescendant(const std::string& name, error_policy ep) {
   if (isName(name.c_str())) return this;
...
void Actor::internalUpdate(const UpdateState& us) {
   ...
   while (tween)
   {
      ...
      if (tween->isDone() && tween->getParentList()) _tweens.remove(tween);
      tween = tweenNext;
   }

   if (_cbDoUpdate) _cbDoUpdate(us);
   doUpdate(us);
...
bool Actor::prepareRender(RenderState& rs, const RenderState& parentRS) {
   if (!(_flags & flag_visible)) return false;
...
bool Actor::internalRender(RenderState& rs, const RenderState& parentRS) {
   if (!prepareRender(rs, parentRS)) return false;
...
void changeParentAndSavePosition(spActor mutualParent, spActor actor, spActor newParent) {
   ...
   if (newParent != mutualParent) pos = convert_global2local(newParent, mutualParent, pos);
...
bool testIntersection(spActor objA, spActor objB, spActor parent, Vector2* contact) {
   ...
   for (int y = 0; y < h; y += HIT_TEST_DOWNSCALE) {
      for (int x = 0; x < w; x += HIT_TEST_DOWNSCALE) {
         ...
         if (contact) *contact = swapped ? posB : posA;
         return true;
      }
   }
