...
namespace oxygine {
std::unordered_set<spHttpRequestTask, spHttpRequestTask::hash> pending_requests;
...
void HttpRequestEmscriptenTask::_onload(void* data, unsigned size) {
   ...
   onComplete();
   pending_requests.erase(this);
   logs::messageln("PENDING REQUESTS: %d", pending_requests.size());
}
