      ...
      for (size_t i = 0; i < b.uniforms.size(); ++i) {
         ...
         switch (uni.tp) {
            case cached_batch::uni::uni_float:
               instance->setUniform(uni.id.c_str(), *((const float*)&uni.data[0]));
               break;
            case cached_batch::uni::uni_int:
               instance->setUniformInt(uni.id.c_str(), *((const int*)&uni.data[0]));
               break;
            ...
            case cached_batch::uni::uni_vec2:
               instance->setUniform(uni.id.c_str(), ((const Vector2*)&uni.data[0]), int(uni.data.size() / sizeof(Vector2)));
               break;
            case cached_batch::uni::uni_vec3:
               instance->setUniform(uni.id.c_str(), ((const Vector3*)&uni.data[0]), int(uni.data.size() / sizeof(Vector3)));
               break;
            case cached_batch::uni::uni_vec4:
               instance->setUniform(uni.id.c_str(), ((const Vector4*)&uni.data[0]), int(uni.data.size() / sizeof(Vector4)));
               break;
            default:
               break;
         }
