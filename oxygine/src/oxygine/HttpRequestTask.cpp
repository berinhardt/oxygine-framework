...
HttpRequestTask::~HttpRequestTask() {
   if (_fhandle) file::close(_fhandle);
}
...
void HttpRequestTask::_finalize(bool error) {
   if (_fhandle) {
      ...
      if (error && !_continueDownload) file::deleteFile(_fname);
   }
   _fhandle = 0;
}
...
void HttpRequestTask::gotHeaders() {
   ...
   if (_continueDownload) asyncProgress(_receivedContentSize, _receivedContentSize, _expectedContentSize);
}

void HttpRequestTask::write(const void* data, size_t size) {
   if (!_suitableResponse) return;
