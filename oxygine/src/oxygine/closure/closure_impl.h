...
extern uint32_t NEXT_FINGERPRINT;
template <TEMPLATE_PARAM_LIST>
struct ClosureBase {
   typedef R (*p_proxy_type)(void* PARAM_FORM_ARG_LIST_COMMA);
   void* p_this;
   p_proxy_type p_proxy;
   uint32_t p_fingerprint;
#if CLOSURE_FUNCTION
   std::function<R(PARAM_TYPE_LIST)> p_function;
#endif

   R operator()(PARAM_FORM_ARG_LIST) {
      if (!p_proxy)
         return p_function(PARAM_ARG_LIST);
      else
...
struct Closure<R(PARAM_TYPE_LIST)> : public detail::CLOSURE_NUM::ClosureBase<R PARAM_TYPE_LIST_COMMA> {
   ...
   Closure(
      ...
      this->p_fingerprint = detail::CLOSURE_NUM::NEXT_FINGERPRINT++;
   }

#if CLOSURE_FUNCTION
   Closure(const std::function<R(PARAM_TYPE_LIST)>& f) {
      this->p_function = f;
      this->p_this = &(this->p_function);
      this->p_proxy = NULL;
      this->p_fingerprint = detail::CLOSURE_NUM::NEXT_FINGERPRINT++;
   }
  ...
  public:
   ...
   bool operator==(const Closure& c) const {
      bool funcs = this->p_proxy || this->p_fingerprint == c.p_fingerprint;
      return this->p_proxy == c.p_proxy && this->p_this == c.p_this && funcs;
   }
};
